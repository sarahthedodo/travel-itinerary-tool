    ...
    <div className="space-y-5 sm:space-y-6 animate-in fade-in duration-300 w-full overflow-x-hidden">
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {plans.map((p, idx) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-[44px] px-3.5 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap flex items-center space-x-2 cursor-pointer ${
                    isActive
          ...
          <button
            ...
            className="min-h-[44px] w-full sm:w-auto px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-semibold shadow-xs flex items-center justify-center space-x-2 transition-colors cursor-pointer shrink-0"
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 text-xs text-slate-600">
          ...
          <div className="flex items-center space-x-3 text-xs">
            <span className="flex items-center space-x-1">
              ...
              <span>{t('common.total')}: <strong className="text-slate-900">{formatCurrency(summary.totalCost, currency)}</strong></span>
            </span>
            <span className="text-slate-300">•</span>
            <span className="flex items-center space-x-1">
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pt-1 pb-1 scrollbar-none">
          <button
            ...
            className={`min-h-[38px] px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap ${
          ...
          {(['flight', 'hotel', 'spot', 'transport', 'food', 'activity'] as const).map((cat) => {
            ...
            return (
              <button
                ...
                className={`min-h-[38px] px-3 py-1.5 rounded-xl text-xs font-medium transition-colors cursor-pointer flex items-center space-x-1.5 whitespace-nowrap ${
                  isSelected
      ...
      {sortedDates.length === 0 ? (
        <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 p-8 sm:p-12 text-center space-y-4">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-base">{t('timeline.noItemsTitle')}</h4>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
            <button
              onClick={() => onOpenAddItem(activePlan.id, activePlan.start_date)}
              className="min-h-[44px] px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold shadow-xs transition-colors cursor-pointer flex items-center space-x-1.5"
            >
              <Plus className="w-4 h-4" />
                ...
                <div className="flex items-center justify-between px-1">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className="min-h-[36px] px-2 text-blue-600 hover:bg-blue-50 rounded-lg text-xs font-semibold flex items-center space-x-1 cursor-pointer"
                      title={t('common.add')}
                    >
                  ...
                  {dayItems.map((item) => {
                    ...
                    return (
                      <div
                        ...
                        className="group bg-white rounded-xl p-3.5 sm:p-4 border border-slate-200/90 shadow-2xs hover:shadow-sm hover:border-blue-300 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 cursor-pointer"
                      >
                        <div className="flex items-start space-x-3 min-w-0">
                          ...
                          <div className="min-w-0 space-y-1">
                            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                              <span
                                className={`px-2 py-0.5 rounded-md text-[11px] font-bold tracking-wide ${meta.bgLight} ${meta.textDark}`}
                              >
                              ...
                              {item.location && (
                                <span className="inline-flex items-center text-xs text-slate-500 truncate max-w-[160px] sm:max-w-xs">
                            ...
                            {item.notes && (
                              <p className="text-xs text-slate-500 flex items-start space-x-1 pt-0.5">
                        ...
                        {/* Cost & Actions */}
                        <div 
                          className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center shrink-0 border-t sm:border-t-0 pt-2 sm:pt-0 border-slate-100 gap-2"
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-1 sm:mt-1">
                            <button
                              onClick={() => setDetailItem(item)}
                              className="min-h-[44px] min-w-[44px] p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors cursor-pointer flex items-center justify-center sm:hidden"
                            ...
                            <button
                              ...
                              className="min-h-[44px] min-w-[44px] p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors cursor-pointer flex items-center justify-center"
                              title={t('common.edit')}
                            >
