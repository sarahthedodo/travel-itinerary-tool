...
export const SqlSetupModal: React.FC<SqlSetupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div 
        id="sql-setup-modal"
        className="w-full sm:max-w-2xl bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]"
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/80 shrink-0">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="font-semibold text-slate-800 text-base">{t('modals.sqlSetup.title')}</h3>
              <p className="text-xs text-slate-500">{t('modals.sqlSetup.subtitle')}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          {/* Free Tier Callout */}
          <div className="bg-emerald-50/80 border border-emerald-200 rounded-2xl p-4 flex items-start space-x-3 text-xs text-emerald-900">
            <ShieldCheck className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
            <div>
              <h4 className="font-semibold">{t('modals.sqlSetup.freeNoticeTitle')}</h4>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="min-h-[38px] px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold flex items-center space-x-1.5 transition-colors cursor-pointer"
              >
                {copied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{copied ? t('modals.sqlSetup.copiedSql') : t('modals.sqlSetup.copySql')}</span>
              </button>
            </div>
            <pre className="p-3.5 bg-slate-900 text-slate-200 rounded-xl text-[11px] font-mono overflow-x-auto max-h-36 leading-relaxed">
              {SUPABASE_SQL_SCRIPT}
            </pre>
          </div>
          ...
          <form onSubmit={handleSaveCredentials} className="space-y-4 pt-2 border-t border-slate-100">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-slate-800 focus:outline-hidden focus:ring-2 focus:ring-emerald-500 text-xs font-mono min-h-[44px]"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3.5 py-2.5 rounded-xl border border-slate-200 text-slate-800 focus:outline-hidden focus:ring-2 focus:ring-emerald-500 text-xs font-mono min-h-[44px]"
                />
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="min-h-[44px] px-3.5 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-semibold flex items-center space-x-1.5 transition-colors cursor-pointer"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="min-h-[44px] text-xs text-blue-600 hover:underline inline-flex items-center space-x-1 px-2 py-2"
                >
              ...
              <div className="flex items-center space-x-2">
                <button
                  type="button"
                  onClick={handleClear}
                  className="min-h-[44px] px-3 py-2 text-xs font-medium text-slate-500 hover:text-slate-800 transition-colors cursor-pointer flex items-center space-x-1"
                >
                ...
                <button
                  type="submit"
                  className="min-h-[44px] px-4 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl shadow-xs transition-colors flex items-center space-x-1.5 cursor-pointer"
                >
